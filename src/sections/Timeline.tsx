  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="absolute left-[-9px] top-1.5 w-[18px] h-[18px] rounded-full bg-bg-card border-2 border-accent-green z-20 flex items-center justify-center"
      >
      ...
      <motion.div
        ...
        className="flex items-center gap-2 mb-2 font-mono text-[10px] text-accent-secondary uppercase tracking-widest"
      >
        <Calendar size={12} />
        <span>{item.date}</span>
      </motion.div>
      ...
      <motion.div
        ...
        className="p-6 md:p-8 rounded-2xl bg-bg-card border border-white/5 hover:border-accent-green/10 hover:shadow-glow-emerald-sm transition-all duration-300 relative overflow-hidden"
      >
        {/* Subtle lab grid watermarks */}
        <div className="absolute top-4 right-6 font-mono text-[8px] text-text-secondary/10 pointer-events-none select-none">
          LAB_LOG_0{index + 1}
        </div>

        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-8 rounded-lg bg-bg-primary border border-white/10 flex items-center justify-center text-accent-secondary">
            <Briefcase size={14} />
          </div>
          <div>
        ...
        <ul className="flex flex-col gap-3">
          {item.bullets.map((bullet, idx) => (
            <li key={idx} className="flex items-start gap-2.5 text-xs text-text-secondary leading-normal">
              <span className="text-accent-green mt-1 text-[8px]">▶</span>
    ...
    <section id="timeline" className="py-24 bg-bg-secondary relative border-y border-white/5">
      ...
      <div className="max-w-3xl mx-auto px-6">
        ...
        >
          ...
          <motion.div 
            className="absolute left-0 top-2 bottom-2 w-[1px] bg-accent-green origin-top z-10"
            style={{ scaleY }}
          />
